            ...
            if source == "sample" or source == "local":
                if not file_path:
                    ...
                    if os.path.exists(user_file):
                        file_path = user_file
                        print(f"Using found user file: {file_path}")
                    else:
        ...
        try:
            ...
            for sheet, raw_df in xls.items():
                ...
                if parsed_df is not None:
                    dfs[sheet] = parsed_df
                    print(f"Sheet '{sheet}' parsed. Columns: {list(parsed_df.columns)}")
            ...
            if 'MOM PL' in dfs:
                ...
                unpivoted['Month'] = pd.to_datetime(unpivoted['Month'], errors='coerce')
