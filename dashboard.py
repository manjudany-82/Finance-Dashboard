...
def _shim_set_query_params(**kwargs):
    # No-op shim: avoid performing any Streamlit UI actions at import time.
    return None


# Runtime-only UI injection: move all CSS/JS into one string so it is
# applied at runtime via `st.markdown(..., unsafe_allow_html=True)`.
_RUNTIME_UI_INJECTION = r"""
<style>
/* UI CSS moved to runtime to avoid import-time Streamlit calls */
/* The original style block preserved below */
.tab-nav-arrow {
    position: absolute;
    top: 8px;
    right: 8px;
...
<script>
// Create tab navigation arrows and re-apply fixes after clicks
if (!document.querySelector('.tab-nav-arrow')) {
    ...
    if (tabsContainer && tabList) {
        ...
        function updateArrowStates() {
            ...
            if (scrollLeft <= 0) leftArrow.classList.add('disabled'); else leftArrow.classList.remove('disabled');
            if (scrollLeft >= maxScroll - 1) rightArrow.classList.add('disabled'); else rightArrow.classList.remove('disabled');
        }
...
function fixDropdowns() {
    ...
    dropdowns.forEach(function(dropdown) {
        ...
        allElements.forEach(function(el) {
            ...
            if (el.style.background && el.style.background.includes('white')) el.style.background = 'transparent';
        });
    });
}
...
def _apply_runtime_ui():
    try:
        st.markdown(_RUNTIME_UI_INJECTION, unsafe_allow_html=True)
    except Exception:
        pass
